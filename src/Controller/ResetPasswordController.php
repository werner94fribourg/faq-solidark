...
class ResetPasswordController extends AbstractController
{
    ...
    public function reset(Request $request, UserPasswordHasherInterface $userPasswordHasher, string $token = null): Response
    {
        ...
        if ($form->isSubmitted()) {
            if($form->isValid())
            {
                ...
                $this->addFlash('password_changed', 'Your password has been changed.');
    ...
    private function processSendingPasswordResetEmail(string $emailFormData, MailerInterface $mailer): RedirectResponse
    {
        ...
        $email = (new TemplatedEmail())
            ->from(new Address('werner94@hotmail.com', 'Reset Password Solidark'))
            ->to($user->getEmail())
            ->subject('Your password reset request')
            ->htmlTemplate('mails/reset_password_email.html.twig')
            ->context([
                'resetToken' => $resetToken,
            ])
        ;
